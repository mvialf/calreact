...
export async function cleanVisitTimes(): Promise<CleanupResult> {
  ...
  try {
    ...
    if (result.success) {
      console.log(`🎉 ¡Limpieza completada exitosamente! ${result.processedCount} visitas actualizadas.`);
    } else {
      console.log(`⚠️ Limpieza completada con errores. ${result.processedCount} visitas actualizadas, ${result.errors.length} errores.`);
    }
...
export function exposeCleanupFunction() {
  if (typeof window !== 'undefined') {
    ...
    console.log('🔧 Función cleanVisitTimes() disponible en window. Ejecuta: await window.cleanVisitTimes()');
  }
}
